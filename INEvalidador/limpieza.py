...
class Limpieza:
    ...
    def escribir_query_sq(self, archivo, comision, nombre, fecha_inicio:datetime="2023-1-1", fecha_final:datetime="2100-12-31") -> str:
        now = datetime.now()
        ...
        #     filtro = filtro.replace("& CP = 0.0","").replace("& CP = nan","").replace("& CP = 0","")
        #     llave_primaria = self.sql.base_col.get(var).replace("_PR","").replace("_SR","").upper() + "-ID"
        #     print(llave_primaria)
        #     filtro = filtro + f"{llave_primaria}"
